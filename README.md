...
> 最新稳定版：`v3.2.9` · [更新日志](./docs/release-notes/v3.2.9.md)<br>
...
|--------------|------------------|----------|--------|----------|----------|
| `latest` | `3.2.9` | Alpine | 3.12 | 精简 | amd64 / arm64 / 386 / arm/v7 |
| `latest-full` | `3.2.9-full` | Alpine | 3.12 | 完整 | amd64 / arm64 / 386 / arm/v7 |
...
Debian 的旧固定版本格式也保留兼容别名：`3.2.9-debian3.10`、`3.2.9-debian3.11`、`3.2.9-debianall` 分别对应新的 `3.2.9-debian-3.10`、`3.2.9-debian-3.11`、`3.2.9-debian-all`。新部署请直接使用新名称。
...
| 看这一版改了什么 | [v3.2.9 更新日志](./docs/release-notes/v3.2.9.md) |
...
- **Docker 完整版**：同样推荐使用 Watchtower。早期直接把 `/var/run/docker.sock` 挂给面板、由面板调用 Docker CLI 更新的部署仍可保留原有 Socket 挂载，但这条兼容更新链只支持完整版标签。使用 `3.2.9-full`、`3.2.9-debian-full` 这类固定完整版标签触发一键更新时，面板会切换到同系列浮动标签 `latest-full` 或 `debian-full`；精简版不包含 Docker CLI。
