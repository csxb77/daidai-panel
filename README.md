...
> 最新稳定版：`v2.3.4` · [更新日志](./docs/release-notes/v2.3.4.md)<br>
> 本次重点：新增 `ddp python` / `ddp shell`（容器内用面板环境跑脚本与交互登录）、补全通知面板名称设置入口、容器统一 UTF-8；修复青龙导入丢失 `task` 前缀、脚本页全屏对话框未真正铺满、实时日志卡顿（v2.3.3 性能回归）等。<br>
