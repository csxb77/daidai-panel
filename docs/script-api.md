...
Node 对 stdout 的写是同步还是异步，**取决于 stdout 连到了什么**（这是 Node 官方 “A note on process I/O” 的口径）：

| stdout 连到 | Windows | Linux / macOS（POSIX） |
|---|---|---|
| 文件 | 同步 | 同步 |
| **管道 / socket** | 同步 | **异步** |
| TTY（终端） | 异步 | 同步 |

面板执行任务时，脚本的 stdout 就是一条**管道**（面板要读它才能显示日志）。所以在 Linux 上 —— 也就是绝大多数 Docker / Magisk 部署 —— `console.log` 返回时字节**还只在进程自己的缓冲区里**，尚未交给内核。

- **自然退出（不调 `process.exit`）不会丢**：事件循环会一直持有这个句柄，等所有待写数据落完才让进程退出。
- **`process.exit()` 会丢**：它不等待，缓冲区里没写完的直接随进程消失。

这不是照抄文档，是实测数字（WSL Linux + node 18.19.1，1.77 MB 输出，每档 40 轮）：
