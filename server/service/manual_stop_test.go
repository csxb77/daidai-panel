...
// 主动停止默认判成功：命中标记 -> 强制成功 + 抑制通知；
// 开启 stop_as_failure 后：命中标记会强制按失败结算，并允许失败通知照旧发送。
func TestApplyManualStopOverride(t *testing.T) {
	...
	const stopAsFailureID uint = 90007
	const stopAsFailureCleanExitID uint = 90008

	// 默认主动停止：即便本次运行结果是失败，也判成功并抑制通知。
	markManualStop(stoppedID)
	run, logStatus, suppress := applyManualStopOverride(stoppedID, &model.Task{}, model.RunFailed, model.LogStatusFailed)
	if run != model.RunSuccess || logStatus != model.LogStatusSuccess || !suppress {
		t.Fatalf("主动停止默认应判成功并抑制通知，got run=%d log=%d suppress=%v", run, logStatus, suppress)
	}
	// 标记应已被消费：再次调用按入参原样返回。
	run, logStatus, suppress = applyManualStopOverride(stoppedID, &model.Task{}, model.RunFailed, model.LogStatusFailed)
	...
	// 用户开启“终止算失败”后：停止强制按失败结算，并允许失败通知照旧发送。
	markManualStop(stopAsFailureID)
	run, logStatus, suppress = applyManualStopOverride(stopAsFailureID, &model.Task{StopAsFailure: true}, model.RunFailed, model.LogStatusFailed)
	if run != model.RunFailed || logStatus != model.LogStatusFailed || suppress {
		t.Fatalf("stop_as_failure=true 时应强制失败且不抑制通知，got run=%d log=%d suppress=%v", run, logStatus, suppress)
	}
	// 即便进程自行退出码为 0，只要命中过主动停止标记且开启“终止算失败”，也要按用户配置写失败。
	markManualStop(stopAsFailureCleanExitID)
	run, logStatus, suppress = applyManualStopOverride(stopAsFailureCleanExitID, &model.Task{StopAsFailure: true}, model.RunSuccess, model.LogStatusSuccess)
	if run != model.RunFailed || logStatus != model.LogStatusFailed || suppress {
		t.Fatalf("stop_as_failure=true 时主动停止应强制失败，got run=%d log=%d suppress=%v", run, logStatus, suppress)
	}

	// 真实失败（未打标记）：保持失败、不抑制通知。
	run, logStatus, suppress = applyManualStopOverride(failedID, &model.Task{}, model.RunFailed, model.LogStatusFailed)
	if run != model.RunFailed || logStatus != model.LogStatusFailed || suppress {
		t.Fatalf("真实失败应保持失败、不抑制通知，got run=%d log=%d suppress=%v", run, logStatus, suppress)
	}

	// 自然成功（未打标记）：保持成功、不抑制通知。
	run, logStatus, suppress = applyManualStopOverride(successID, &model.Task{}, model.RunSuccess, model.LogStatusSuccess)
	if run != model.RunSuccess || logStatus != model.LogStatusSuccess || suppress {
