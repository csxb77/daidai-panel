      ...
      <div class="stat-cards">
        ...
        <div class="stat-card">
          ...
          <div class="stat-card__sparkline">
            <svg viewBox="0 0 60 24" preserveAspectRatio="none">
              <polyline
                points="0,14 10,10 20,12 30,6 40,8 50,4 60,5"
                fill="none"
                stroke="#409eff"
                stroke-width="2"
                stroke-linecap="round"
      ...
      <div class="trend-chart-card">
        <div class="trend-chart-card__header">
          ...
          <div class="trend-chart-card__legend">
            <span
              class="trend-chart-card__legend-item trend-chart-card__legend-item--success"
              ><span class="legend-dot"></span>成功</span
            >
            <span
              class="trend-chart-card__legend-item trend-chart-card__legend-item--fail"
              ><span class="legend-dot"></span>失败</span
            >
          </div>
        </div>
        <div class="trend-chart-card__body">
          ...
          >
            ...
            <polygon
              points="0,60 40,45 80,50 120,30 160,35 200,20 240,25 280,15 280,100 0,100"
            ...
            />
            <polygon
              points="0,85 40,80 80,82 120,75 160,78 200,72 240,76 280,70 280,100 0,100"
              fill="url(#apiFailGrad)"
            />
            <polyline
              points="0,85 40,80 80,82 120,75 160,78 200,72 240,76 280,70"
              fill="none"
              stroke="#f56c6c"
              stroke-width="2"
              stroke-linecap="round"
...
const apiStats = computed(() => {
  ...
  const totalRequests = apps.value.reduce(
    (sum: number, a: any) => sum + (Number(a.call_count) || 0),
    0,
  );
  const rateLimits = apps.value
    .filter((a: any) => Number(a.rate_limit) > 0)
    .map((a: any) => Number(a.rate_limit));
